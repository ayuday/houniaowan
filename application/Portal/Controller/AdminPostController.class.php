...
class AdminPostController extends AdminbaseController {
	...
	public function edit(){
		...
		//by ayumi 自定义模板
		if(empty($post['one_tpl']))
		{
			//by ayumi 栏目id
			$data=$this->terms_model->where(array("term_id" => $term_relationship[0]))->find();
			$this->assign("one_tpl",$data['one_tpl']);
			
		}
		else
		{
			
			$this->assign("one_tpl",$post['one_tpl']);
		}
		//end by ayumi 自定义模板
	...
	public function edit_post(){
		if (IS_POST) {
			...
			//by ayumi自定义模板
			$term_relationship = $this->term_relationships_model->where(array("object_id"=>$post_id,"status"=>1))->getField("term_id",true);
			$data=$this->terms_model->where(array("term_id" => $term_relationship[0]))->find();
			
			if($data['one_tpl']==$_POST['one_tpl']) {
				$_POST['post']['one_tpl'] = null;
			}
			else{ 
				$_POST['post']['one_tpl'] = $_POST['one_tpl'];
			}
			//end by ayumi自定义模板
